...
import { PLAN_BASE_PRICE, PLAN_ORDER } from '@/context/SubscriptionContext';
  ...
  const load = async () => {
    try {
      const [u, ps, pp, asu, approved, settings] = await withTimeout(Promise.all([
        ...
        supabase.from('subscription_requests').select('plan_type, amount_tk').eq('status', 'approved'),
      ...
      PLAN_ORDER.forEach(p => { breakdown[p] = { count: 0, total: 0 }; });
      let total = 0;
      (approved.data || []).forEach((r: any) => {
        // amount_tk is the actual charge (already accounts for storage level);
        // fall back to the base price only for old rows recorded before that column existed.
        const price = r.amount_tk ?? PLAN_BASE_PRICE[r.plan_type as keyof typeof PLAN_BASE_PRICE] ?? 0;
        if (!breakdown[r.plan_type]) breakdown[r.plan_type] = { count: 0, total: 0 };
        breakdown[r.plan_type].count++; breakdown[r.plan_type].total += price;
        total += price;
      });
      setEarnings(total);
