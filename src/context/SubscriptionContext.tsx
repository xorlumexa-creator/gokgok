...
export const PLAN_BASE_PRICE: Record<PlanId, number> = {
  basic: 80,
  standard: 120,
  pro: 180,
};
...
interface SubscriptionState {
  plan: PlanId;
  storageLevel: number; // 1, 2, 3...
  startedAt: string | null;
  expiresAt: string | null;
  salesCreditUsed: number;
  salesCreditPeriod: string; // YYYY-MM
}
...
interface SubscriptionContextType extends SubscriptionState {
  ...
  guardAddProduct: () => boolean; // returns true if allowed; otherwise opens modal
...
function defaultState(): SubscriptionState {
  return {
    ...
    expiresAt: null,
    salesCreditUsed: 0,
    salesCreditPeriod: currentPeriod(),
  };
}
...
export function SubscriptionProvider({ children }: { children: ReactNode }) {
  ...
  const [state, setState] = useState<SubscriptionState>(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed.salesCreditPeriod !== period) {
          parsed.salesCreditPeriod = period;
          parsed.salesCreditUsed = 0;
        }
  ...
  const isPlanActive = useMemo(() => {
    if (!state.expiresAt) return true; // grace until first subscribe
  ...
  const guardAddProduct = useCallback(() => {
    if (products.length >= productLimit) {
      openLock({ type: 'product_limit', current: products.length, limit: productLimit });
      return false;
    }
    return true;
  ...
  const guardAddCustomer = useCallback(() => {
    if (customers.length >= bakiLimit) {
      openLock({ type: 'baki_limit', current: customers.length, limit: bakiLimit });
      return false;
    }
    return true;
  ...
  const guardRecordSale = useCallback((count = 1) => {
    if ((state.salesCreditUsed + count) > salesCreditLimit) {
      openLock({ type: 'sales_credit', used: state.salesCreditUsed, limit: salesCreditLimit });
      return false;
    }
    return true;
  ...
  const guardFeature = useCallback((feature: 'whatsapp' | 'invoice') => {
    if (!hasFeature(feature)) {
      openLock({ type: feature === 'whatsapp' ? 'feature_whatsapp' : 'feature_invoice' });
      return false;
    }
    return true;
  }, [hasFeature, openLock]);
