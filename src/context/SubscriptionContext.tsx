...
export function SubscriptionProvider({ children }: { children: ReactNode }) {
  ...
  // FIX: refresh() previously only ran once on mount. If the app is just
  // backgrounded (switched away from) rather than force-closed, this
  // component never remounts — so it could keep serving stale data
  // (e.g. an old plan_expiry) for as long as the app sits paused in
  // Android's memory, even after a manager approved a renewal server-side.
  // Re-fetching on every resume/focus keeps it honest without waiting for
  // a full app restart.
  useEffect(() => {
